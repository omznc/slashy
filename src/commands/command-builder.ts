...
const options = {
  ...
  ephemeral:
    ...
    (option: SlashCommandBooleanOption) =>
      option
        .setName("ephemeral")
        .setDescription("Will the reply be invisible to everyone else?")
        .setRequired(required),
  role:
    (required = false) =>
    (option: SlashCommandRoleOption) =>
      option
        .setName("role")
        .setDescription("Role required to manage Slashy.")
        .setRequired(required),
};
...
class Commands {
  ...
  async clear() {
    await rest
      .put(Routes.applicationCommands(config.DISCORD_CLIENT_ID), { body: [] })
  ...
  async initialPublish() {
    await rest
      .get(Routes.applicationCommands(config.DISCORD_CLIENT_ID))
      .then(async (commands: any) => {
        if (commands.length === 0) {
          logger.info("No Global (/) Commands found, publishing...");
          await this.publish().then(() =>
            logger.info("Published Global (/) Commands")
          );
        }
      })
      .catch((err: Error) => logger.error(err));
...
const SlashyCommands = new Commands(
  new SlashCommandBuilder()
    ...
    // HELP
    .addSubcommand((subcommand: SlashCommandSubcommandBuilder) =>
      subcommand.setName("help").setDescription("Show this help message.")
    )
...
SlashyCommands.command.options.forEach((option: any) =>
  subcommands.push(`slashy ${option.name}`)
);

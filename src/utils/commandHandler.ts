...
const _config = require("./configLoader").getConfigs([ConfigTypes.DEVELOPERS]);
...
function getOptions(interaction: CommandInteraction): Map<string, any> {
  return new Map<string, any>(
    ...
    ).data[0].options?.map((option: CommandInteractionOption) => [
      option.name,
      option.value,
    ])
  );
...
async function checkPrerequisites(
  ...
  return isInGuild(interaction).then(async (inGuild) => {
    if (!inGuild) return false;
    return hasPermission(interaction).then((hasPermissions) => hasPermissions);
  });
}
 ...
 */
function getPlaceholders(interaction: CommandInteraction): {
  [key: string]: string;
} {
  return {
...
async function ParsePlaceholders(
  ...
  for (const [key, value] of Object.entries(getPlaceholders(interaction)))
    reply = reply.replaceAll(key, value);
...
export const handleAutocomplete = async (
  ...
  const guildCommandNames = (await GetGuildCommands(interaction.guildId)).map(
 ...
 */
export const handleGlobalCommand = async (
  interaction: CommandInteraction
): Promise<void> => {
  ...
  switch (
    ...
    case "add":
      if (await checkPrerequisites(interaction))
        await add(interaction, getOptions(interaction));
      break;

    case "remove":
      if (await checkPrerequisites(interaction))
        await remove(interaction, getOptions(interaction));
      break;

    case "edit":
      if (await checkPrerequisites(interaction))
        await edit(interaction, getOptions(interaction));
      break;

    case "config":
      if (await checkPrerequisites(interaction))
        await config(interaction, getOptions(interaction));
      break;
...
export const handleUserCommand = async (
  ...
  await GetGuildCommand(interaction.guildId!, interaction.commandName)
    .then(async (command) => {

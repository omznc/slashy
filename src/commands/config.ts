...
const _config = require("@utils/configLoader").getConfigs([
  ConfigTypes.LOGO,
  ConfigTypes.COLOR,
]);
...
export const config = async (
  ...
  await GetGuildBanned(interaction.guildId!)
    ...
    .then(async () => {
      ...
      switch (role) {
        case undefined:
          const [maxCommands, currentCommands] = await GetGuildCommandLimitInfo(
            interaction.guildId!
          );
                ...
                .addFields([
                  {
                    ...
                    value:
                      oldRole === "OWNER"
                        ? messages.CommandConfigPermissionIsOwner
                        : messages.CommandConfigPermissionIsRole.replace(
                            "{oldRole}",
                            oldRole
                          ),
                  },
                  {
                    name: "Command Limits",
                    value: messages.CommandConfigCommandLimits.replace(
                      "{maxCommands}",
                      maxCommands.toString()
                    ).replace("{currentCommands}", currentCommands.toString()),
                  },
                ]),
            ],
          });
        ...
        case oldRole:
          await interaction.editReply(
            messages.CommandConfigSamePermissionError
          );
          break;
        default:
          await SetGuildPermission(interaction.guildId!, role).then(
            async () => {
              await interaction.editReply(
                messages.CommandConfigSuccess.replace("{role}", `<@&${role}>`)
              );
            }
          );
          break;

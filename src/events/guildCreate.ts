							...
							.addFields([
								{
									...
									value: "Run `/slashy add` and provide it the inputs it needs to create your command.",
								},
								{
									name: "How to run a command?",
									value: "Just do / and your command's name. It's available instantly as soon as you create it.",
								},
								{
									name: "How do I setup permissions?",
									value: "This is incredibly easy to do. By default, only the server owner can use Slashy's add, remove, edit & config commands. You can use `/slashy config [role]` with any new or existing role to use that as a required role to modify custom commands in your server.\nOnly the server owner can run this command - that's you!\n__All users can run custom commands.__",
								},
							]),
					],
					components: [
						new ActionRowBuilder<ButtonBuilder>().setComponents([
							...
							new ButtonBuilder()
								.setURL(
									"https://hey.imkez.com/slashy-documentation"
								)

...
import { DoesCommandExist, EditGuildCommand, GetGuildBanned } from "../database/methods";
...
export const edit = async (
	...
	await GetGuildBanned(interaction.guildId!)
		...
		.then(async () => {
			...
			if (reply != undefined && !isValidReply(reply)) return Promise.reject(messages.CommandReplyInvalidError);

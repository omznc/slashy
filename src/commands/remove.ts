...
import { RemoveGuildCommandAPI } from "@utils/commandAPI";
  ...
  await GetGuildBanned(interaction.guildId!)
    ...
    .then(async () => {
      ...
      // Remove the command
        ...
        await RemoveGuildCommand(interaction.guildId!, name).then(
          async (commandId) => {
            if (commandId === null)
              return Promise.reject(messages.UnspecifiedError);
            await RemoveGuildCommandAPI(interaction.guildId!, commandId).then(
              async () => {
                await interaction.editReply(messages.CommandRemoved);
                if (config.EXTRA_LOGGING)
                  logger.info(
                    `[COMMAND] Removed ${name} from guild ${interaction.guildId}`
                  );
              }
            );
          }

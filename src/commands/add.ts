...
import {
	RegisterGuildCommandAPI,
	RemoveGuildCommandAPI,
} from "@utils/commands-api";
...
export const add = async (
	...
	await GetGuildBanned(interaction.guildId!)
		.then((isBanned) => {
			if (isBanned)
				return Promise.reject(messages.CommandGuildBannedError);
		})
		.then(async () => {
			await GetGuildCommandLimitInfo(interaction.guildId!).then(
				([max, current]) => {
					if (current >= max)
						return Promise.reject(
							messages.CommandLimitReachedSlashy.replace(
								"{max}",
								max.toString()
							)
						);
		...
		.then(async () => {
			...
			if (
				...
				(await DoesCommandExist(interaction.guildId!, name))
			)
				return Promise.reject(messages.CommandNameInvalidError);
			...
			const description: string =
				options.get("description") ?? DefaultDescription;
			...
			await RegisterGuildCommandAPI(
					...
					await AddGuildCommand(
						...
						.then(async () => {
							...
							if (config.EXTRA_LOGGING)
								logger.info(
									`[COMMAND] Added ${name} to guild ${interaction.guildId}`
								);
						})
						.catch(async (error) => {
							await RemoveGuildCommandAPI(
								interaction.guildId!,
								command.id
							);
							logger.error(error);
				...
				.catch((err: DiscordAPIError | Error) => {
					if (err instanceof DiscordAPIError)
						if (err.code === 30032)
							return Promise.reject(
								messages.CommandLimitReachedDiscord
							);
					logger.error(err);
